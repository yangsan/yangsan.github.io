...
你至少应该能够配置python的环境，能够顺利的安装python的插件。
...
所以你应该看明白了，必须要通过每篇博文开头的部分来告诉pelican这篇文章的标题、标签、分类等信息，以便最终呈现在博客页面上。其中，标题、时间几项是必填的，如果缺失将无法被系统转换成网页。
...
这里所谓的静态页面指的是此刻博客页面添头上'about'这样的页面，添加方法同样简单。只需要在content目录下再新建一个pages的页面，将你想要展示的内容放进去就行，格式和普通的博客文章相同。
...
如果你想要一个个性的域名，那就不得不破费一下了，推荐去[Godaddy]()购买，可以用支付宝支付。
之后
